...
const SLICE=220; // Limit the number of entries to max num of ticks
...
export default function TimelineGraph({ data }) {
  const options = useMemo(() => {
    ...
    for (let i = 0; i < SLICE; i++) {
      const tickId = i.toString();
      const tickData = data[tickId];
      const [p = 0, s = 0, f = 0, e = 0] = tickData?.status || [0, 0, 0, 0];
